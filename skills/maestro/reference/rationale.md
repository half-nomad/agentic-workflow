...
## 계획서의 사실 주장을 확인하게 한 이유 (2026-08-08 승격, 실운영 6런)

`maestro-runs.md` 6런의 `놓친 것` 열을 교차 분석하니 **같은 실패가 4런에서 반복**됐다 — 계획서에 *확인하지 않은 사실*을 적는 것:

- "인플루언서 XLSX 도 같은 결함"이라 적었으나 20줄 위에 이미 검사하는 테스트가 있었다 (그 오판의 연장으로 **방어선인 테스트를 군더더기라며 지웠다**)
- "깨질 테스트 3건" → 실제 7건 이상
- "`border-base-content/30` 4곳 유지" → `app/views` 기준 3곳. **멀쩡한 코드를 결함으로 오판할 기대치를 심을 뻔했다**
- 계정의 이름 칸을 `name` 으로 적었으나 실재하지 않는 컬럼이었고(`display_name`/`username`), 유일 인덱스 때문에 두 번째 파기 계정부터 실패할 뻔했다

전부 Codex 계획 검토나 리뷰어가 사후에 정정했다. **계획서의 숫자는 곧 검증 기대치가 되므로, 틀린 사실은 검증 자체를 오염시킨다** — 목표 1(사용자가 계획을 먼저 본다)이 무너지는 지점이 여기다. 3문항 게이트: 목표 1 방어 ✓ / 훅으로 못 잡음 / 4회 반복 ✓.

같은 분석에서 나온 2/6·3/6 패턴은 아직 승격하지 않고 `known-failure-modes.md` §관찰 중에 뒀다.
