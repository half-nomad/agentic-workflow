...
> **왜 지금**: 실사용 프로젝트 한 곳의 33런(2026-08-07~08-22) 을 클로드 네이티브 기능과 대조한 결과, 마에스트로 절차의 상당 부분이 **이제 기본 기능과 중복**이라는 것이 확인됐다. plan mode · 서브에이전트 위임 · Workflow 툴 · `/code-review` · 완료 보고 정직성은 전부 네이티브다.
...
**이 워크플로가 자기를 재는 유일한 근거가 백업 없이 로컬에만 있었다.** 실사용 프로젝트의 `.gitignore` 가 `/.agentic/` 을 *"외부 에이전트 워크스페이스"* 로 분류해 무시하고 있었는데, **마에스트로가 거기에 런 기록을 쓴다는 걸 모른 채였다.** 33런 — duet 실험의 기준선(*사용자 지적 7/33 = 21%*)과 v5.3.0 결정 전체의 근거 — 이 지우면 영구 소실인 상태였다(2026-08-23 발견).
