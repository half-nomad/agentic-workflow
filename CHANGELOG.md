...
### Fixed — `## Next Session` 이 상태가 아니라 일지가 되던 원인

`skills/maestro/SKILL.md` §On Completion 의 템플릿이 **이력을 직접 요구하고 있었다**:
...
`Summary: 달성한 것` 은 정의상 이력이다. **묻는 칸이 있으면 채우게 되고, 채우면 다음 세션이 이미 지나간 것을 재개 지점으로 읽는다.** 실제로 이 블록이 50줄까지 자랐고 그 대부분이 `CHANGELOG.md`·스킬 파일에 정본이 있는 내용의 복제였다.

- **템플릿을 상태 전용으로 교체** — `Task`(아직 안 끝난 것) / `Next`(다음 세션의 첫 행동) / `Blocker`(없으면 생략). 이어받을 게 없으면 한 줄로 비운다.
- **`Status:`·`Summary:` 금지를 이유와 함께 명시** — 이유 없이 지우면 다음 편집자가 되살린다.
- **블록 5줄 상한** — MEMORY.md 는 잘림 임계점이 있는 **인덱스**이지 일지가 아니다. 길어지면 정본을 딴 데 두고 가리킨다.
- **하위 절(`### ...`) 보존 규칙 추가** — 장기 상태를 세션 요약이 쓸어버리지 않게. 이 트리엔 git 이력이 없어 잘못 지우면 영구 소실이다.

> **왜 이제야 잡혔나 — silent drift.** 같은 계약이 두 곳에 inline 복제돼 있었고 **한쪽만 고쳐져 있었다.** 다른 쪽은 이미 *"그 두 줄이 이 블록을 상태에서 이력으로 바꿔 놓은 원인"* 이라고 진단해 자기 템플릿을 고쳤는데, 이 저장소의 사본은 옛 템플릿 그대로 배포되고 있었다. `memory-management` 룰 §Drift 경보가 경고하는 정확히 그 형태다.
