...
- 프롬프트는 **stdin 또는 `--prompt-file` 로만.** 단일 인자 `task "..."` 는 재토크나이즈로 인용부호·개행이 소실된다.
...
- **로그 정지 ≠ 좀비.** 로그가 수십 분 안 움직여도 긴 추론 구간일 수 있다. 취소 전에 `result` 를 먼저 시도하고, 좀비 판정은 런타임 재시작 정황이 있을 때만.
