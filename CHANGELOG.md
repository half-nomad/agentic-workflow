...
### Added — v5.3.0 (2026-08-23): `/duet` 경량 pilot 모드 · 마에스트로 절차 의례 정리 · 두 훈계 표 삭제

> **왜 지금**: 33런(2026-08-07~08-22, `실사용 프로젝트의 런 기록`) 을 클로드 네이티브 기능과 대조한 결과, 마에스트로 절차의 상당 부분이 **이제 기본 기능과 중복**이라는 것이 확인됐다. plan mode · 서브에이전트 위임 · Workflow 툴 · `/code-review` · 완료 보고 정직성은 전부 네이티브다.
> **그런데 넷은 중복이 아니었다**: 타벤더 교차검증(코덱스) · 훅으로 강제되는 저자/검증자 분리 · mode A(깨뜨려 확인) · 검증 축 0이면 완료 금지. 33런 중 8런에 *"검토들이 겹치지 않았다"* 가 명시돼 있고, **32/33런의 `놓친 것` 다수가 오케스트레이터 자신의 계획 사실 오류·위임 지침 구멍**이었다 — 자기가 틀렸다는 건 자기가 못 본다.
> **그래서**: 중복인 층은 걷어내고, 중복이 아닌 넷 중 *가장 싼 둘*(코덱스 관문 2회)만 남긴 경량 모드를 새로 만든다. 마에스트로는 그대로 둔다.

- **`/duet` 신설** (`skills/duet/SKILL.md`, 자족 파일 1개 — 참조 디렉터리·훅·상주 룰 없음) — **네이티브 plan mode 에 코덱스 관문 두 개를 붙인 것.**
  - **관문 하나(설계)**: 계획 초안을 승인 요청 **전에** 코덱스에 넘겨 *반증* 과 **대안**("이 계획이 틀렸다면 무엇을 대신 하겠나")을 함께 받는다. 반박만 받으면 검토이고, 대안까지 받아야 설계 참여다. 채택 여부는 오케스트레이터가 판단한다.
  - **관문 둘(적대 검토)** — **두 축을 분리했다**: `A-read`(읽기 적대 리뷰, 항상 안전, 기본) 와 `A-mut`(변형 검사 — 구현을 틀리게 바꿔 테스트가 죽는지 확인). **A-mut 은 사용자 작업 트리를 훼손하지 않는다**: 격리 worktree 1순위 → 원 트리 + 복원 스냅샷(`cmp` 바이트 대조) → **둘 다 못 하면 A-mut 을 하지 않고 `N/A — 격리 불가`**. 작업물이 검사보다 중요하다.
  - **고치는 것은 코덱스가 아니라 클로드다** — `클로드 구현 → 코덱스가 결함과 재현법 보고 → 클로드가 재현·수정 → 테스트 재실행`. 코덱스가 본 구현을 고치면 **검증자와 저자가 다시 섞여** 이 모드의 유일한 독립 축이 그 자리에서 사라진다. 코덱스 출력은 수정 지시가 아니라 **수정 입력**이다. 재현 안 되는 지적은 기각하고 사유를 남긴다.
  - **`@architect` 제거는 결론이 아니라 실험이다** — 같은 벤더라도 전문 역할 지침 · 깨끗한 별도 컨텍스트 · 설계 전제 재구성 · 앵커링 완화라는 값이 남는다. 다른 벤더라는 이유만으로 항상 열등하지 않다.
  - **그 사이는 전부 자율** — 클로드가 직접 짜고 직접 테스트한다. 위임 금지 훅 없음, `@architect` 의무 없음, 절차표 없음.
  - **위임 강제를 뺀 근거**: 목표 2의 목적은 *저자와 검증자의 분리* 였는데 기본 위임 대상이 **또 다른 클로드**였다 — 워크플로 자신의 논리(*"축은 사람이 아니라 방식으로 센다"*)대로면 약한 분리다. 코덱스가 더 강하게 대신한다. 그리고 위임 자체가 결함군을 만들고 있었다(20·28·32·33번 런: *"위임 지침에 구멍 셋"* · *"내 위임 계약이 죽은 코드를 만들게 했다"*).
  - **라우팅은 위험·불확실성으로 가른다 — 파일 수로 나누지 않는다.** 20개 파일의 기계적 rename 은 저위험이고 한 파일의 인증 변경은 고위험이다. `/duet` 기준(하나라도): 불확실한 설계 판단 · 모듈 경계/상태/공개 계약에 영향 · 독립 검토가 실제로 유용 · 되돌릴 수 있으나 재작업 비용이 큼. 되돌리기 어려운 것(보안·결제·운영 데이터·마이그레이션·배포·공개 API·외부 발송)은 `/maestro` 에 남는다. 작업 중 성격이 드러나면 그 자리에서 승격.
  - **판정은 동등성 시험이 아니라 pilot 이다** — 8~10런은 결함 1건이 10%p 라 품질 동등성을 가릴 수 없다. 이 구간의 목적은 **위험 신호와 사용성**이고, *즉시 복구 트리거*로 운영한다: **ownership·invariant·failure mode 관련 중대 설계 누락이 1건이라도 나오면 `@architect` 의무 호출 즉시 복원**(비율을 기다리지 않는다) · A-mut 이 작업물을 훼손하면 worktree 전용 고정 · 코덱스가 본 구현을 고치면 의뢰서에 수정 금지 명시. 기록은 *확인된 actionable finding · 결정을 바꿨나 · 오탐 · 놓친 것(단순) · 중대 설계 누락(별도) · 호출·시간* 으로 센다.
  - **관문이 못 잡은 런이 많다는 것 자체는 무용의 증거가 아니다** — 원래 구현이 깨끗했을 수 있다. 무용의 증거는 *"결정을 바꾼 적이 없는데 오탐과 시간은 들었다"* 다.

> **이 설계 자체를 교차벤더 관문에 두 번 통과시켰다.** 1차 — 지적 4건 중 3건이 실제 결함(파일 수 라우팅 · 적대 검토가 읽기/변형을 섞고 작업 트리를 훼손 가능 · 판정 기준 느슨함). 2차 — `@architect` 를 뺀 자리의 **관찰자 부재**(아래 그림자 검토)와 첫 데이터포인트의 과대 표현을 짚었다.
>
> **표현을 정정한다**: 이것은 *"관문의 값이 실증됐다"* 가 아니라 **"첫 스모크 테스트에서 유효 신호가 확인됐다"** 다. 확인된 지적 정밀도 3/4(75%) · 표본은 설계 1건 · **재현율은 측정 불가**(관문이 놓친 결함 수를 모른다). **파일럿을 시작할 근거로는 충분하고, `@architect` 제거를 확정할 근거로는 부족하다.**
>
> **33런이 증명한 것과 아닌 것도 구분한다** — 증명된 것: *서로 다른 검토 축이 유용하다*(8런 "겹치지 않았다" · 13런 "결정적"). **증명되지 않은 것**: 코덱스 관문이 `@architect` 를 대체할 수 있는가 · 교차벤더 프리미엄의 크기 · 관문의 오탐·지연 비용 · duet 이라는 새 흐름에서 같은 효과가 재현되는가. **그래서 세 번째 사용자 모드는 안 만들되, duet 내부에서 관문을 단계별로 계측한다** — 이 둘은 양립한다.

### Fixed — 런 기록이 gitignore 로 사라지고 있었다 (`.agentic/` → `.claude/agent-runs/`)

**이 워크플로가 자기를 재는 유일한 근거가 백업 없이 로컬에만 있었다.** `실사용 프로젝트의 .gitignore` 가 `/.agentic/` 을 *"External agent tool workspaces (Codex, OpenAI Agents SDK 등)"* 로 분류해 무시하고 있었는데, **마에스트로가 거기에 런 기록을 쓴다는 걸 모른 채였다.** 33런 — duet 실험의 기준선(*사용자 지적 7/33 = 21%*)과 v5.3.0 결정 전체의 근거 — 이 지우면 영구 소실인 상태였다(2026-08-23 발견).

- **런 기록 경로를 `.claude/agent-runs/<mode>-runs.md` 로 옮겼다.** `.claude/` 는 프로젝트가 에이전트 설정을 두는 곳이라 이미 추적되고 있고, 무시 대상이 될 이유가 없다. 모드 상태 파일(`.agentic/maestro-mode.state`)은 진짜 일회성이라 **그대로 둔다** — 훅 3개가 그 경로에 의존한다.
- **`maestro-guard` 훅도 함께 고쳤다** — 경로만 옮기면 **오케스트레이터가 마에스트로 모드에서 자기 런 기록을 못 쓰게 된다**(가드가 `.agentic/` 만 허용했다). `.claude/agent-runs/*.md` 를 좁게 앵커해 허용하고 `.sh`·`.ps1` 양쪽에 넣었다. 가짜 프로젝트로 6/6 검증 — 런 기록 허용, 코드·`.claude/settings.json`·`agent-runs` 밖 파일은 여전히 차단.
- **재발 방지 — 쓰기 전에 무시 여부를 확인한다**: `git check-ignore -q <경로>` 로 확인하고 무시되면 추적되는 곳으로 옮긴다. `WORKFLOW.md` §기록과 duet §기록 양쪽에 넣었다. **경로를 한 번 고르는 것으로는 부족하다** — 다음 프로젝트의 `.gitignore` 가 무엇을 무시할지는 우리가 정하지 않는다.
- **`docs/` 에 두지 않은 이유** — 처음엔 `docs/agent-runs/` 로 옮겼는데 `bin/docs-check` 가 문서 안의 경로 참조를 검사해 막혔다. **런 기록은 그 시점의 경로를 담는 역사 기록**이라 린터에 맞춰 고치면 기록이 왜곡된다. (다만 그 검사는 추적되는 파일 전체를 보므로 위치를 옮겨도 걸린다 — 결국 참조 2건을 정직하게 정정했다: 완료돼 `docs/active/` → `docs/specs/` 로 이동한 문서는 두 경로를 병기하고, 다른 레포 경로를 로컬처럼 적은 것은 서술로 바꿨다.)

### Added — 그림자 검토 (`@architect`, 첫 3~5런 한정) · 관문별 계측

- **그림자 검토** — `@architect` 를 뺀 것은 실험인데 **관찰자가 없었다.** 코덱스와 클로드가 *둘 다* 놓친 설계 누락은 정의상 자기 관찰이 안 되므로, 복구 트리거(`중대 누락 1건`)가 **사용자 피해나 장애가 나야** 작동하는 상태였다. 완료 후 `@architect` 가 결과에 개입하지 않고 훑는다 — fix-loop 를 열지 않고, 다음 런부터 복구 여부만 정한다. (훈계 표 때는 대조군만으로 충분했다. **그쪽 발동은 자기 관찰이 되고 설계 누락은 안 되기 때문**이다.)
  - **하인드사이트 표시** — 그림자는 완성물을 보므로 계획 단계의 `@architect` 보다 유리하다. 발견마다 *"설계 단계에서 잡을 수 있었나"* 를 함께 적지 않으면 복원 근거가 못 된다.
  - **비용 계측에서 제외** — 그림자를 붙인 런은 호출 수가 부풀려져 있다. 마에스트로 대비 비교는 6번째 런부터.
- **관문별 계측** — 기록을 `관문1 (findings / 바꾼 결정)` · `관문2 (findings / 바꾼 결정)` 로 쪼갰다. 사용자에게 보이는 명령이 하나여도 계측은 단계별로 한다.
- **중대 누락의 확정 요건** — **클로드가 재현했거나 `@architect` 가 확인한 것만** 센다. 코덱스나 그림자의 주장만으로는 세지 않는다 — 이 칸이 `@architect` 복원 트리거를 당기므로 **오탐 하나가 실험을 끝내버린다.**

### Unchanged — `A-mut` 의 스냅샷 경로는 유지 (교차검토 제안 중 유일한 기각)

*"원본 작업 트리 스냅샷 방식은 원칙적으로 사용하지 않는다"* 는 제안은 **받지 않았다.** worktree 격리가 1순위인 것은 그대로지만, **훅이 자격증명 파일 복사를 거부해 worktree 를 못 만드는 환경이 실재한다**(2026-08-17 런). worktree 전용으로 고정하면 그런 환경에서 A-mut 이 **영구히 불가능**해지는데, A-mut 은 *"61개 테스트가 전부 통과하는데 아무것도 안 지키던 것"* 을 잡은 축이고 지문 대조는 *코덱스가 되돌리기를 잘못된 위치에 넣은 것*을 실제로 잡았다(2026-08-16 런). **검증된 축을 이미 완화된 위험을 이유로 버리는 거래**라 기각한다.

대신 스냅샷 경로에 전제 셋을 걸었다: worktree 가 **왜** 막혔는지 확인 · 실행 **전에** 스냅샷을 떠서 복원 가능함을 확인 · 끝나면 `cmp` 로 **바이트 일치** 확인. 셋 중 하나라도 못 하면 A-mut 을 하지 않는다.

### Removed — 두 훈계 표 (`Soft Violation Guard` · `Rationalization 반박표`)

v5.0 이 **주입을 멈추고 관찰**하기로 하면서 못 박은 판정은 *"10런에 발동 0건이면 삭제"* 였다. **33런에서 발동 0건** — 기준을 세 배 넘겼으므로 삭제한다. `reference/known-failure-modes.md` 7,818 → 3,516 바이트(−55%), 86 → 31줄.

- **원문은 `docs/maestro-v4.5-rules-archive.md` 에 그대로 있다** — 재발하면 복원 비용은 한 번의 편집이다.
- 두 표가 덮던 것 중 **지금도 유효한 것은 이미 `WORKFLOW.md` 강제 규약에 있다** (검증 축 0 · anomaly 조사 기본값 · Codex 직접 호출 · WORKFLOW 재읽기) — 가드 슬롯이 비지 않는다.
- `§발동 기록` 절도 함께 삭제 — 그 절의 목적이 두 표의 측정이었다. `§관찰 중`(승격 후보)과 2026-08-13 실패 기록 2건은 **남긴다**.

### Changed — `reference/phases.md` 의 절차 의례를 판단으로 교체 (173 → 149줄, 바이트는 −6%)

**크기 감축이 목적이 아니다** — 바이트는 6% 밖에 안 줄었고, 줄어든 24줄은 전부 *절차*다. 네이티브가 이미 하는 일을 다시 적은 자리만 걷어내고 판단만 남겼다. **판단이 담긴 곳은 한 줄도 안 건드렸다** — Anomaly Comparator · 공격형 가드레일 4항 · R1 first match · mode A 선택 기준 · Codex fallback 한계는 네이티브가 하지 않는 것이라 그대로다.

- **Phase 2** — 가용 에이전트 3위치 스캔·목록 작성 절차 삭제. 시스템 리마인더가 이미 노출한다. 남긴 판단 둘: `Domain match preempts global`, auto-delegate 금지.
- **Phase 3** — plan mode 6단계 재서술 삭제. `EnterPlanMode`/`ExitPlanMode` 가 뼈대이고, 그 위에 얹는 것만 남겼다(clean context 위임은 선택 · Plan agent 는 Task tool 이 없어 오케스트레이터가 @architect 를 직접 호출 · plan 헤더 표기).
- **Phase 6** — 조건표 삭제. `verify-*` 스킬이 있을 때만 별도 값이 있고, 없으면 완료 보고가 그 역할을 흡수한다. 채점표(`rubrics/success-criteria.md`) 포인터는 유지.

### Unchanged — 자동 발동 전환은 보류

`제안 후 승인` 누적이 3건(수락 2 · 미선택 1)뿐이고 전환 기준은 10건이었다. 판단이 어긋난 적은 없지만 표본이 부족하다. 지금 구조를 유지한다.


### Added — v5.2.0 (2026-08-21): 시각 축의 렌더 수단 · 외부 사실 확인 축(옵션)

- **시각 축에 렌더 수단을 적었다** — 종전에는 `rubrics/visual-axis.md` 가 *"실제 렌더가 필수"* 라고만 하고 **무엇으로 렌더할지는 아무 데도 없었다.** 도구 선택이 사용자의 개인 전역 룰에 의존하고 있었으므로, 이 저장소를 설치한 사람에게는 요구만 있고 수단이 없었다. 우선순위 3단(프로젝트 지정 > 세션에 붙은 브라우저 도구 > 헤드리스)과 **상태를 먼저 확인하고 잰다**(로그인 여부에 따라 V5·V8 이 뒤집힌다), **수단이 없으면 PASS 가 아니라 `N/A — 렌더 수단 없음`** 을 명시했다. 특정 도구는 지정하지 않는다 — 환경마다 다르다.
- **외부 사실 확인 축 추가 (옵션, 기본 off)** — 계획서의 사실 주장 규약은 *코드로 확인 가능한 것*만 덮는다. 외부 API 사양·라이브러리 실동작·요금·서드파티 문서 같은 **코드 밖 사실**은 어떤 축도 덮지 않았다. modifier(`"웹에서 확인"` · `"어사이드"` 등)로만 켜고, 없는데 켜면 축이 아니라 지연이므로 기본값은 off. 직접 호출 · 시간 상한 · 실패 시 `N/A` 규정을 함께 둔다.


### Changed — v5.1.0 (2026-08-13): 작업자 프로파일 · 위임 지침의 배경 절 · mode A 의 방법 명시

> **되돌리기**: 이 변경 전체가 커밋 하나이므로 `git revert` 로 원복되고, 배포본이 심볼릭 링크라 되돌리는 즉시 반영된다. 부분 해제는 `worker=claude` 트리거로 프로파일만 끌 수 있다.

- **작업자 프로파일 추가** — 자연어(`"구현은 코덱스로"` · `worker=codex`)로 구현 워커를 Codex 로 바꾸고, `"원래대로"` · `worker=claude` 로 되돌린다. **목표 1~4 · 나머지 규약 · 사용자 대면 형식은 불변이고 중간층만 교체된다** — 계획·승인·검증 종합·완료 보고는 어느 경우에도 오케스트레이터가 한다. 켤 때 따라오는 것 셋: 위임 지침을 `--prompt-file` 로 · 만든 스레드와 검사하는 스레드 분리(`--fresh`) · 같은 작업 폴더에 쓰는 작업은 줄 세우기.
- **위임 지침에 배경 절 필수** — *무엇이 기존 구조이고 무엇이 이번 변경인가.* 빼면 맥락 없는 워커와 검증자가 **원래 그랬던 것을 이번 결함으로 읽는다.** 두 번 반복된 실패의 승격이다 (2026-08-13 차단급 5건이 전부 기존 발송의 원래 성질 · 2026-08-12 정본에 이미 있던 이름을 못 보고 새 이름 생성).
- **mode A 의 방법 명시** — 읽어서 판단하는 게 아니라 **구현을 일부러 망가뜨려 테스트가 실제로 죽는지 확인**하고, 죽지 않는 자리를 산출로 보고한다. 실행 권한을 뺏으면 읽기 검토와 같은 축이 되므로 **권한 회수 금지** — 동시 실행이 곤란하면 순서로 푼다.
- **검증 축을 사람이 아니라 방식으로 센다** — 검증자가 둘이어도 둘 다 코드를 읽었으면 1축이다. 종전에 용어 사전에만 있던 정의를 §3 규약 자리에 명시했다. 사람 수로 세면 커버리지가 실제보다 넓어 보이고 그 과대 보고가 곧 목표 3의 실패다.
- **작업자별 검증 배치표** — 맥락이 필요한 축(리뷰)은 프로젝트를 아는 쪽에, 저자의 맹점을 찌르는 축(mode A)은 저자와 다른 모델에. 작업자 = Claude 면 리뷰 `@code-reviewer` / mode A Codex, 작업자 = Codex 면 리뷰 Codex 새 스레드(배경 절 필수) / mode A `@code-reviewer`·`@architect`.
- **`reference/known-failure-modes.md` 에 실패 2건 기록** — 검증자를 늘리면 수정 라운드가 늘어난다(모아서 넘기지 않은 탓, 현실적 최소는 2라운드) · 미러할 모범 파일을 찾았다는 건 N번째 복제라는 뜻(본문 10분 대 장치 80분, 일반화 항목은 TODO 에 이미 있었다).
- **마에스트로 가드의 범위를 현재 프로젝트로 제한** — 편집 대상이 프로젝트 밖이면 막지 않는다. 한 세션이 켠 모드가 다른 레포·전역 설정 작업까지 가두던 문제다 (2026-08-13 실제 발생). 프로젝트 루트를 확정하지 못하면 예외를 적용하지 않는다(fail closed).
...
>
> **판정축이 바뀌었다**: 종전 — *"이 지시가 있으면 더 안전한가"* (답이 거의 항상 yes 라 래칫이 된다) → v5.0 — **"이 지시가 없으면 목표가 깨지는가"** (아니면 규약이 아니라 참고).
>
> **되돌리기**: 이전 상주 룰 원문 전체 → [`docs/maestro-v4.5-rules-archive.md`](docs/maestro-v4.5-rules-archive.md). 내려간 절차는 전부 `skills/maestro/reference/` 에 원문 그대로 있다 — **삭제가 아니라 계층화다.**

- **목표 4개를 문서에 명시** — 종전에는 어디에도 목표 정의가 없었고, 그게 래칫의 뿌리였다(기준이 없으니 사고마다 지시만 쌓였다). 이제 명시된다: (1) 사용자가 계획을 먼저 본다 (2) 직접 짜지 않고 위임한다 (3) "확인했다"에 증거가 따른다 (4) 되돌리기 어려운 결정엔 다른 관점이 붙는다. **이 넷이 동시에 모든 규약의 판정 기준이다.**
- **강제는 목표당 최소선 하나씩** — 나머지(5 Effect 표 · Phase 절차 6단계 · mode T/A 구분 · R1 first match · fix-loop 횟수 · Anomaly Comparator 절차표 · Delegation 상세)는 전부 `reference/` 로. 지시가 아니라 **참고**이며, 목표를 만족한다면 다른 방법을 써도 된다.
- **매 런 로드 35,447자 → 12,904자 (−63.6%)** — 상주 스텁 2,919 + WORKFLOW 6,272 + SKILL 3,713. 필요할 때만 읽는 참조가 23,759자.
- **`reference/` 신설 6개** — `phases.md`(단계 상세) · `delegation.md`(위임·plan 템플릿) · `verification-commands.md`(framework 커맨드) · `codex-cli.md`(호출 문법) · `rationale.md`(규약의 근거·실운영 사례) · `known-failure-modes.md`(두 훈계 표).
- **규약을 추가하기 전 3문항 게이트 신설** (래칫 방지) — (1) 목표 1~4 중 무엇을 지키는가? 답할 수 없으면 참고다 (2) 훅으로 잡을 수 있는가? 잡히면 프롬프트가 아니라 훅으로 (3) 한 번의 사고인가 반복인가? 한 번이면 기록만, 반복될 때 승격. **한 번의 실수가 영구 지시가 되면 지시 총량이 늘고 개별 구속력이 떨어진다.**
- **기록 장치를 "자율의 이익 측정"으로** — `.agentic/maestro-runs.md` 에 런당 `다르게 한 것 / 그래서 나았나 / 놓친 것`. `놓친 것` 이 반복되는 지점은 참고에서 강제로 **승격**하고, `다르게 한 것` 이 반복해서 나은 지점은 참고 자체를 고친다. 양방향이다.
- **교차검증(Codex)은 줄이지 않고 재배치 — 계획은 기본 on, 구현은 표적** — 초안에서는 "오판 비용이 큰 지점에만"으로 좁혔다가(계획: Hard rule·비가역 변경 / 구현: 4개 표면) **되돌렸다.** 실사용에서 설계·리뷰 비중이 1:1 이고 **계획 단계는 대부분 교차검증이 필요했다**는 관찰 때문이다. 좁은 조건은 실제로 이 v5.0 재설계 작업조차 자동 호출 0회로 판정했는데, 파급이 넓은 설계 결정이라 명백히 필요한 경우였다 — `되돌리기 어려운` 을 *물리적 비가역*(migration·삭제)으로만 해석한 정의 오류였다.
  - **계획 적대 검토**: complex 로 계획을 만들었으면 기본 on. 끄는 경우는 셋뿐 — 사용자 modifier · 확립된 패턴의 단순 확장 · simple 판정. **계획이 틀리면 그 뒤 작업이 전부 낭비이므로 여기가 가장 싸고 이익이 크다.**
  - **구현 공격(mode A)**: 기존 4표면 + **이 런에서 절차와 다르게 판단한 것이 있을 때**. 구현 공격은 이미 만든 것이 대상이라 늦고 비싸므로 실적이 확인된 표면에만 건다.
- **자율↑ 이면 검증은 유지하거나 강화한다 (설계 원칙)** — 절차를 참고로 내려 판단에 맡긴 만큼, 그 판단이 맞았는지 확인하는 독립 축이 더 필요해진다. **지시를 줄이면서 검증도 같이 줄이면 남는 건 확인되지 않은 자율이다.** 그래서 자율을 쓴 런(`다르게 한 것` ≠ 없음)에는 교차검증을 자동으로 붙인다 — 자율의 이익을 측정하겠다면 그 측정에 독립적인 관점이 있어야 한다.

- **`rules/maestro-workflow.md` 348줄(18,770자) → ~50줄 스텁** — 판정 기준·절차·출력 계약·검증 규약은 `skills/maestro/WORKFLOW.md`(지연 로드)로 이동. 상주에 남은 것은 활성화 조건과 **절대 규칙 4개**뿐이다. 상주의 근거는 *"compact 후에도 유지"* 였는데, **그 문제는 이 저장소의 `hooks/maestro-compact-reload.sh`(PostCompact)가 이미 해결하고 있었다** — 훅이 maestro 모드일 때만 WORKFLOW.md 재읽기를 재주입한다. 근거가 자기 레포 안의 훅에 의해 이미 해소돼 있던 셈이다. 효과: `/maestro` 를 쓰지 않는 세션(대부분)의 상주 비용이 사라진다. 훅의 재주입 문구도 *"구속 룰은 rules 에 그대로 살아 있다"* 에서 *"상주분은 스텁이므로 그것만으로 진행하지 말 것"* 으로 교정했다 — 스텁화로 그 문장이 거짓이 됐다.
- **절대 규칙에 "검증 축이 없으면 완료를 선언하지 않는다" 추가** — 종전 룰의 검증 척추(5b/5c/5d)는 전부 `tests`/`lint`/`build` axis 위에 서 있어서, **테스트가 없는 프로젝트에서는 전 축이 `N/A` 로 통과하고 6단계 의례를 거친 뒤 `— 작업 완료 —` 가 출력됐다.** 검증이 0인데 검증된 형태의 확신을 주는 것이라 없느니만 못했다. 이제 그 경우 완료 대신 *무엇을 확인하지 못했는지* 와 *사용자가 직접 확인할 것* 을 보고한다. **검증 수단 부재는 완료의 사유가 아니라 미완료의 내용이다.**
- **사용자 보고 규약 신설 (`WORKFLOW.md` §사용자 보고)** — 내부 용어를 숨기지도, 그냥 던지지도 않는다: 그 보고에서 처음 나올 때 `용어(쉬운 설명)` 로 한 번 풀고 이후엔 용어만 쓴다. 용어를 지우면 다음에도 못 알아듣고, 그냥 던지면 지금 못 알아듣는다. 자주 쓰는 10개(`5c 풀 스위트`·`axis`·`mode A`·`fix-loop` …)의 풀어쓰기 사전을 함께 뒀다. 결과 보고는 **만든 것 / 확인한 방법(숫자로) / 확인 못 한 것 / 사용자가 할 일** 4항.
- **Phase 2 패턴 표 삭제** (Chaining / Parallelization / Routing / Orchestrator-Workers / Evaluator) — Anthropic *"Building effective agents"*(2024-12) 어휘를 그대로 채택한 것인데, 패턴 이름을 고르는 절차가 산출물을 바꾸지 않는다. 의존이 있으면 순차, 없으면 병렬 — 위임 계획을 짜면 자연히 결정되는 것에 이름표를 붙이는 어휘세였다. Phase 2 는 *에이전트·스킬 스캔* 으로 남는다. `workflow 위임 게이트`의 트리거 술어도 패턴 이름 대신 *"독립 항목의 병렬 실행"* 으로 교체.
- **Phase 1 복잡도 판정표 삭제** (Steps 1-2/3+ · Files 1-2/3+ · Domains) — 기계적이라 오분류를 유도했다("파일 3개면 무조건 complex"). *여러 파일 · 여러 도메인 · 되돌리기 어려운 변경 중 하나라도 해당하면 complex* 한 줄로 대체.
- **Codex 교차검증을 조건화 — 루틴 complex 런에서 호출 2회 → 0회** — 종전에는 complex 판정만으로 Codex#1·#2 를 자동 2회 호출했다. 실적이 확인된 축은 mode A(구현 공격, 실결함 2건 회수 기록)였고 루틴 청크의 mode T 는 5d Reviewer 와 중복이었다. 이제 **Codex#1** 은 Hard rule 영역이거나 되돌리기 어려운 변경(migration·데이터 삭제/이동·배포 스크립트·공개 API)일 때, **Codex#2** 는 mode A 대상(공유 컴포넌트·클라이언트 상태 로직·요청 간 상태 이동·Hard rule 인접)일 때만 자동 호출한다. Elevated risk 강제와 사용자 modifier(`"교차 검증"`)는 그대로.

### Changed — 두 훈계 표: 삭제도 존치도 아닌 "주입 중단 + 관찰"

`Soft Violation Guard` 와 `Rationalization 반박표` 를 **`reference/known-failure-modes.md` 로 내렸다.** 내용은 한 줄도 지우지 않았고, 매 런 주입만 멈췄다.

두 표의 원 근거는 v3.3.1 시절의 사용자 관찰(*"네가 silent-skip 을 무분별하게 재량껏 하길래 없애버림"*)이다. **그 관찰의 대상 모델은 지금 모델이 아니다.** 근거가 낡았다는 것과 룰이 불필요하다는 것은 다른 주장이므로 실측으로 가른다 — 그런데 **매 런 주입하는 한 그 표가 일을 하는지 알 수 없다.** 주입을 멈춘 상태가 곧 대조군이다.

- **측정 대상은 "재발이 없었다" 가 아니라 "이 표가 나를 붙잡은 적이 있는가"** — 전자는 표가 없어도 참일 수 있어 아무것도 증명하지 못한다.
- **기록은 붙잡힌 그 순간에** — 완료 시점에 회상하면 놓친다. 되돌린 판단은 기억에 남지 않는다.
- **판정**: 10런에 발동 0건 → 삭제 / 특정 항목만 반복 발동 → **그 항목만 `WORKFLOW.md` 강제 규약으로 승격** / `놓친 것` 반복 → 해당 상황을 추가 후 승격.
- **재발하면 되돌린다.** 아카이브와 참조 원문이 그대로 있어 복원 비용은 한 번의 편집이다.

두 표의 *의도적 중복* 은 5쌍으로 유지된다 (`검증 0 상태` 추가). 2026-07-29 병합 기각 결정도 유효하다 — 다만 이제 상주가 아니다.

### Fixed — 배포 스크립트의 파괴적 경로 9건 (적대 리뷰 발견 · 전건 재현 후 수정)

> 공통 불변식: **install 은 복구 가능한 백업 없이 아무것도 파괴하지 않는다 · uninstall 은 이 레포가 배포하지 않은 것을 제거하는 게 *구조적으로* 불가능하다.** 아래는 전부 이 불변식을 깨고 있던 경로다. 모두 가짜 `$HOME`(`$TMPDIR` 하위)에서 재현 → 수정 → 회귀 하네스로 확정 (shell 59/59 · pwsh 40/40 · ps1 훅 9/9. 수정 전 동일 하네스는 shell 19 FAIL · pwsh 15 FAIL).

- **`uninstall.ps1` 이 manifest 경로를 무조건 신뢰 — 홈 밖 임의 디렉터리 삭제** — manifest 한 줄을 `Join-Path $userHome` 한 뒤 곧장 `Remove-Item -Recurse -Force` 했다. `..\..\Documents` 한 줄이면 배포 루트 밖의 실제 디렉터리가 통째로 사라진다(재현: 바로 그 한 줄로 `~/shouldnotdelete/` 삭제 확인). 수정 — 해석 전에 `..` 세그먼트를 `[\\/]` 양쪽 구분자로 거르고, `GetFullPath()` 로 정규화한 뒤 **구분자 경계까지 포함해** `~\.claude` · `~\.codex` 안쪽인지 확인한다(`~\.claudeX` 는 통과 못 함). 실패한 항목은 삭제하지 않고 경고 후 건너뛴다.
- **`install.ps1` 이 manifest 에 적힌 경로 하나로 "지금도 우리 것"을 단정** — 사용자가 `skills\maestro` 를 자기 내용으로 갈아끼운 뒤 재설치하면, 백업 없이 재귀 삭제됐다. **경로 이름은 현재 소유권의 증거가 아니다.** 수정 — 배포 시점의 **콘텐츠 지문**을 함께 기록한다(파일: 바이트 SHA256 / 디렉터리: 정렬된 `<상대경로><TAB><SHA256>` 목록의 SHA256). 재설치·제거 시 지문이 어긋나면 **삭제하지 않고** 백업(install)하거나 건너뛴다(uninstall). manifest 는 `<SHA256>  <경로>` 형식으로 읽을 수 있게 유지하고 포맷을 파일 머리 주석에 명시했다. 지문 없는 구형 manifest 줄은 "검증 불가"로 취급 — 아무 소유권도 주장하지 못한다.
- **`install.sh` `link()` 가 대상이 다른 심볼릭 링크를 백업 없이 파괴** — `~/.claude/hooks/verify-prompt.sh -> ~/personal/verify.sh` 같은 사용자 자신의 별칭을 `rm` 했다. 링크의 내용은 링크 텍스트가 전부이므로 이건 영구 소실이고, 뒤이은 `ln -s` 가 실패하면 되돌릴 것도 남지 않는다. 수정 — 일반 파일과 **동일하게** 백업 경로로 보낸다(`mv` 는 링크 자체를 옮겨 대상 텍스트를 보존한다 — 백업 디렉터리에서 `readlink` 로 확인). 우리가 방금 건 링크와 대상이 일치할 때만 무동작.
- **`guard_claude_md` 가 심볼릭 링크*이기만* 하면 소유권 가드를 면제** — `~/.claude/CLAUDE.md -> ~/personal/CLAUDE.md` 가 가드를 통째로 우회했고, 그 다음 `link()` 가 그걸 지웠다. 수정 — 면제는 **이미 `$REPO/CLAUDE.md` 로 해석되는 링크**(멱등 재설치)뿐. 그 외 링크는 깨진 것 포함 전부 사용자 소유로 보고 같은 보호를 받는다 — 실제 파일로 해석되고 마커 밖에 내용이 있으면 동일 메시지로 중단.
- **마커 토폴로지 미검증 — 개인 지침이 조용히 대체됨** — 가드가 `BEGIN 있음 AND END 있음` 만 보고 awk 상태 기계를 돌렸다. **재현**: `END` → `BEGIN` → 개인 텍스트 순서인 파일은 leftover 가 비어 나와 install 이 그대로 진행하고 개인 텍스트를 덮었다. `BEGIN` 중복도 같은 방식으로 모호하다. 수정 — `BEGIN` 정확히 1개 · `END` 정확히 1개 · `BEGIN` 이 `END` 앞, 이 셋을 모두 만족할 때만 해석한다. 그 외 토폴로지(중복·역순·한쪽만)는 "마커가 손상됐으니 직접 정리하라"는 메시지와 함께 중단하고 파일은 건드리지 않는다. `install.ps1` 의 동일 결함도 같은 규칙으로 교정.
- **`claude-md-sync` 가 대상이 다른(또는 깨진) `~/.codex/AGENTS.md` 링크를 *통과해* 씀** — 가드가 `! [ ... -ef "$SIBLING" ]` 였다. 이 조건은 링크가 **다른 데를 가리킬 때 참**이므로, `~/.codex/AGENTS.md -> ~/important.md` 를 둔 사용자는 전역 `CLAUDE.md` 를 편집할 때마다 `important.md` 가 덮어써졌다(깨진 링크면 없던 대상 파일이 새로 생겼다). 기존 주석은 `[ -L ]` 단독 검사가 왜 부족한지는 옳게 설명했지만 채택한 수정이 정확히 반대 구멍이었다. 수정 — **목적지가 무엇인지**로 판정한다: 링크가 아니면 우리 것(Windows 복사 설치, 이 분기의 존재 이유) → 쓴다 / `$SIBLING` 로 해석되는 링크면 분기 1이 이미 썼다 → 조용히 건너뛴다 / **그 외 링크는 깨진 것 포함 전부** → stderr 경고 후 건너뛴다. `.ps1` 도 동일 규칙으로 교정.
- **`claude-md-sync.sh` 가 `realpath` 부재 시 추측으로 레포 추적 파일을 덮어씀** — 배포된 링크를 거쳐 편집이 들어왔는데 `realpath` 가 없으면 `$SIBLING` 이 존재하지도 않는 `~/.claude/AGENTS.md` 가 되고, `-ef` 가 거짓이 되어 분기 2 가 (정상인) Codex 링크를 통과해 레포의 추적 대상 `AGENTS.md` 에 썼다. 수정 — `~/.claude/CLAUDE.md` 가 링크인데 해석이 불가능하면 추측하지 말고 분기 2 를 건너뛴다.
- **`find` 가 루트 자신을 매치 — `~/.claude` 전체 삭제 가능** — `uninstall.sh` 와 `install.sh` 의 dangling 스윕 모두 `-mindepth` 없이 `find "$root" -maxdepth 2 -type l` 이었다. `~/.claude` 자체가 레포 안을 가리키는 심볼릭 링크면 find 가 루트를 보고하고 `rm` 이 사용자 설정 디렉터리 링크를 통째로 지운다. 양쪽 `find` 에 `-mindepth 1` 추가.
- **dangling 스윕이 사용자가 만든 링크까지 수거** — "레포를 가리키는 죽은 링크"는 너무 느슨한 기준이다(`~/.claude/rules/scratch.md -> $REPO/docs/notes.md` 도 해당된다). 수정 — 이 설치기가 **바로 그 자리에 바로 그 대상으로** 걸었을 링크만 수거하도록 배포 레이아웃 매핑(`expected_target`)에 대조한다.
- **실패한 `ln` 이후의 복구가 남의 파일을 덮어쓸 수 있었음** — 되돌리는 `mv` 가 그 사이 다른 프로세스가 만든 목적지를 무조건 밀어냈다. 수정 — 목적지가 **아직 비어 있을 때만** 복구하고, 아니면 백업을 그대로 두고 충돌을 보고한다.
- **`backup()` 실패 메시지의 "nothing else was changed" 가 거짓** — 앞선 링크들이 이미 걸린 뒤라면 사실이 아니다. "이 경로는 옮기지 않았고, 앞서 처리한 경로는 이미 바뀌었다"로 교정.
...
- **`README.md` 의 마이그레이션 3절(39줄)을 `docs/migrations.md` 로 이관** — README 는 **처음 설치하는 외부 사용자**를 위한 문서인데 v2.x→v3.0, v3.0→v3.1, v3.1→v4.1 전환표가 본문 한가운데 있었다. 이력은 `docs/` 로 보내고 README 상단에 한 줄 링크만 남긴다. v4.1→v4.5(복사→심볼릭 링크, 배포 범위 축소) 절도 새로 추가.
- **`README.md` 특징 목록에서 내부 버전 표기 제거** — `(v4.0)`·`(v3.3)`·`(v4.1, research-preview)` 는 이 저장소의 개발 이력이지 기능 설명이 아니다. 처음 보는 사람에게는 의미가 없다.
- **`docs/` 5개는 전부 유지한다** — 전부 시점 기록이다. `maestro-summary.md` 는 `**Date**:` 헤더가 없어 현재 서술처럼 읽히던 것이 문제였으므로, 나머지 4개와 같은 형식(`Date` / `Purpose` / `Status`)의 헤더를 붙여 시점 기록임을 명시했다. 레거시 용어가 많은 것(`maestro-v4-overoptimization-analysis.md` 33회)은 결함이 아니라 *그 날짜의 시스템을 정확히 서술한 것*이다.
- **예외 1건: `docs/maestro-hybrid-feasibility.md`** — 날짜 헤더로 해결되지 않는 문제가 하나 있다. probe *전* 가설과 *후* 결론이 **한 파일 안에 공존**해 §3 과 §7 이 서로 반대되는 말을 한다. 날짜는 "언제 썼나"를 말해줄 뿐 "문서 안에서 뒤가 이긴다"를 말해주지 못하므로, 그 한 줄만 상단에 남겼다.
...
- **판단 근거** — 두 언어에 걸친 **152줄, 배포 코드 전체의 ~19%** 이자 유일하게 중복 구현된 덩어리였는데, 정작 막고 있던 건 데이터 소실이 아니라 **백업당하는 불편**이었다. 실제 위험(사용자 지시문 소실)은 다른 21개 경로가 이미 쓰는 규칙 — 실제 파일이 자리에 있으면 `.maestro-backup-<타임스탬프>/` 로 크게 알리며 옮긴다 — 이 그대로 막는다. "아무도 안 읽는 백업은 조용한 소실" 이라는 원래 논거는 `rules/global.md` 나 에이전트 파일에도 똑같이 적용되는데 거기서는 백업으로 충분하다고 받아들이고 있었다 — `CLAUDE.md` 만 특례일 이유가 없다.
- **남긴 것은 3줄** — 실제 파일이 백업될 때에 한해 `BACKED UP:` 뒤에 "`~/.claude/CLAUDE.md` 는 이제 이 레포로의 링크이고, 개인 전역 지시는 `rules/personal.md` 로" 안내를 출력한다. 밀려난 사용자가 가장 헷갈리기 쉬운 단 하나의 파일이라 안내만 남기고 차단은 없앴다. 멱등 재설치(백업 없음)에서는 출력되지 않는다.
- **함께 사라진 것** — `BEGIN_MARK`/`END_MARK` 상수, awk·PowerShell 마커 토폴로지 상태 기계, 선두 BOM 제거, 배포본 바이트 동일성 비교, 두 호출 지점. 마커 블록은 심볼릭 링크 전환 시점에 이미 없어졌으므로 파싱 대상 자체가 존재하지 않는다.
- **문서 동반 수정** — `README.md` §주의사항 4항 · `CLAUDE.md` §Rules 의 "중단된다" 서술을 백업 서술로 교체.
...
- **`skills/maestro/SKILL.md` §Skill Candidates 의 `codex-image` 예시 제거** — 같은 배치의 죽은 참조 정리에서 빠졌던 7번째. 나머지 예시(`notion-*` · `pdf` · `verify-*` · `note-*`)는 독자가 얻을 수 있는 스킬을 가리키지만 `codex-image` 는 이제 개인 저장소에만 있어, 공개 문서가 **독자가 구할 수 없는 것**을 후보로 제안하게 된다. 같은 배치가 `CLAUDE.md` 쪽 언급은 이미 익명화했으므로 내부 정합성 문제이기도 하다.
...
- **`docs/maestro-summary.md` v1.8 이력 항목 소급 수정 복원** — 이 배치가 해당 줄을 현재형 문구(`프로젝트가 verify-* 스킬을 제공하면 …`)로 **교체**해 v1.8 이 실제로 무엇을 했는지의 기록을 지웠다. 같은 릴리스가 `CHANGELOG` 에 대해서는 *"기존 항목은 판단 기록이라 남긴다"* 를 명시했으므로 한쪽만 소급 수정하는 건 자기모순이다. 원문(`verify-*` 스킬(manage-skills, verify-implementation) 연동)을 되살리고 현재 상태는 **괄호로 부기**한다 — 기록은 보존하되 이 레포가 그 스킬을 배포한다는 오해는 남기지 않는다.
- **`README.md` 푸터 버전을 `CLAUDE.md` 와 동기화** — `v4.1.4` → `v4.5.0`. 두 진입 문서가 서로 다른 버전을 주장하고 있었다.
