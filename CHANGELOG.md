...
### Added — v5.3.0 (2026-08-23): `/duet` 경량 모드 · 마에스트로 절차 의례 정리 · 두 훈계 표 삭제
...
- **`/duet` 신설** (`skills/duet/SKILL.md`, 자족 파일 1개 — 참조 디렉터리·훅·상주 룰 없음) — **네이티브 plan mode 에 코덱스 관문 두 개를 붙인 것.**
  ...
  - **관문 둘(적대 mode A)**: 구현 diff 를 깨뜨려 테스트가 실제로 죽는지 확인. 실행 권한을 뺏지 않는다 — 빼면 이 모드에 남은 유일한 독립 축이 사라진다. 미커밋 작업물은 `git diff` 스냅샷 + `cmp` 로 원복 확인.
  ...
  - **라우팅 3층**: 파일 1~2개·되돌릴 수 있음 → 기본 / 여러 파일·**되돌릴 수 있음** → `/duet` / **되돌리기 어려움**(마이그레이션·데이터 삭제·이동·배포·공개 API·외부 발송) → `/maestro`. 작업 중 성격이 드러나면 그 자리에서 승격한다.
  - **판정 기준을 사전 등록했다** (8~10런 후) — 기준선은 마에스트로 33런의 *사용자·오너가 직접 결함을 지적한 런 = 7/33 = 21%*. 30% 이하면 유지 / 30% 초과 + 설계 결함이면 `@architect` 의무 복원 / 관문 둘 다 `없음` 인 런이 절반 넘으면 라우팅 기준 상향. **결과를 보고 기준을 정하면 그건 측정이 아니라 합리화다.**
