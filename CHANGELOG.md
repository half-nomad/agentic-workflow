...
### Fixed — `## Next Session` 의 고정 필드를 전부 걷어냄 (앞선 수정이 원인이었다)

> **왜 다시**: 2026-08-23 (`d93ef47`) 에 `Status:`·`Summary:` 를 "이력 유발 원인" 으로 진단해 지우고 `Task:`·`Next:`·`Blocker:` 를 대신 넣었다. 그런데 **그 셋도 같은 병이었다.** `Task: <아직 끝나지 않은 작업>` 은 주어가 *작업*이라, 성실히 채우면 "이번 세션이 무엇을 하다 말았나" 가 들어간다. 실사용에서 정확히 그렇게 났다 — 블록에 이번 세션의 작업 서술 2줄이 다시 실렸고, 그중 하나는 **쓰는 순간 이미 거짓**이었다(동시 세션이 이미 배포를 마친 상태였다).
> **진단**: 문제는 어느 칸이냐가 아니라 **칸이 있다는 것**이다. 빈칸은 채우라는 지시로 읽히고, 채울 것이 없으면 이번 런에서 있었던 일을 가져다 채운다. 산문으로 "여긴 이력이 아니라 상태다" 를 세 번 적어도 템플릿이 이긴다 — 실제로 채우는 건 템플릿이기 때문이다.

- **고정 필드 전부 삭제.** 템플릿을 주지 않고 *무엇을 인계할지는 스스로 판단하게* 한다. 인계할 것이 없으면 `- (없음)` 한 줄.
- **양이 많으면 좌표만 찍는다** — 문서 경로 + 절 이름. 본문은 그쪽이 갖는다. (종전 "5줄 상한" 이 하던 일을 규칙이 아니라 방향으로 바꾼 것.)
- **통째 재작성 → 줄 단위 수정.** 동시 세션이 같은 `MEMORY.md` 를 쓰는데 종전 지침이 `Replace ... in place` 였다. 실사용에서 옆 세션이 남긴 줄을 판단만으로 지운 사례가 났다.
- **규칙과 함께 *이유* 를 적었다.** `d93ef47` 이 규칙만 바꾸고 이유를 안 남겨서, 다음 편집자(모델)가 같은 모양의 칸을 되살렸다. 이유 없는 규칙은 되살아난다.
