# [보존] Maestro v4.5.0 상주 구속 룰 — 원문

**보존일**: 2026-08-07 · **원본**: `rules/maestro-workflow.md` (commit `e44017d` 시점, 348줄 / 18,770자)

## 왜 남겼나

v5.0.0 에서 이 파일은 **~50줄 상주 스텁**으로 축소되고, 판정 기준·절차·출력 계약·검증 규약은 `skills/maestro/WORKFLOW.md` (지연 로드) 로 내려갔다. 상주 근거였던 *"compact 후에도 유지"* 는 이 저장소의 `hooks/maestro-compact-reload.sh` (PostCompact) 가 이미 해결하고 있었다.

축소 과정에서 **삭제된 것**은 다음 둘뿐이고, 나머지는 전부 WORKFLOW.md 로 이동했다:

1. **Phase 2 패턴 표** (Chaining / Parallelization / Routing / Orchestrator-Workers / Evaluator) — 패턴 이름을 고르는 절차가 산출물을 바꾸지 않아 제거. 의존이 있으면 순차, 없으면 병렬이라는 판단만 남겼다
2. **Phase 1 복잡도 판정표** (Steps / Files / Domains 수치 기준) — 기계적이라 오분류를 유도했다. 한 줄 판단으로 대체

**Codex trigger 는 삭제가 아니라 조건화**됐다 (complex 자동 2회 → 오판 비용이 큰 지점만). 원래 조건은 아래 §Codex Integration 에 그대로 있다.

## 되돌릴 때

이 파일 내용을 `rules/maestro-workflow.md` 에 그대로 쓰고 `./install.sh` 를 다시 실행하면 v4.5.0 동작으로 돌아간다. 부분 복원이면 해당 절만 상주로 올린다 — 단 **WORKFLOW.md 에 같은 내용이 남아 있으면 중복**이므로 한쪽을 링크로 압축할 것.

판단 근거와 측정 계획은 `CHANGELOG.md` v5.0.0 항목 참조.
