...
- **`docs/maestro-summary.md` 삭제 (344줄)** — 섹션이 `README.md` 와 그대로 겹치는 중복 요약인데, `/ultrawork`·`/swarm`·`/ralph` 를 *현재 동작하는 모드* 로 서술하는 v1.8 세계관이었다. **`docs/` 5개 중 유일하게 `**Date**:` 헤더가 없어** 시점 기록이 아니라 현재 서술로 읽혔다. 배너를 붙여 살려두는 선택지는 기각 — 읽는 사람에게 "믿을 수 없는 문서"를 하나 더 남길 뿐이고, 그 역할은 README 가 이미 한다. 인바운드 참조 1곳(`maestro-vs-dynamic-workflows.md:41`)은 README 를 가리키도록 변경.
- **나머지 `docs/` 4개는 손대지 않는다** — 전부 `**Date**:` 헤더를 갖고 있어 그 자체로 시점 기록으로 읽힌다. 레거시 용어가 많은 것(`maestro-v4-overoptimization-analysis.md` 33회)은 결함이 아니라 *그 날짜의 시스템을 정확히 서술한 것*이다. README 의 디렉터리 설명만 "시점 기록 — 현재 동작은 `rules/` 와 README 가 정본"으로 바꿨다.
