...
<!-- Also read ~/.claude/rules/*.md (secure-coding, global, ...) and apply those rules identically when working here. -->
...
> `~/.claude/CLAUDE.md` 는 이 레포 `CLAUDE.md` 로의 심볼릭 링크다 — 더 이상 관리 블록이 없다. 그 파일을 직접 고치면 이 레포를 고치는 것과 같다 (레포 관리자에겐 의도된 동작, 개인 메모를 적으려는 목적이면 잘못된 자리). 설치는 `~/.claude/CLAUDE.md` 에 사용자 고유 내용이 남아 있으면 덮어쓰지 않고 **중단**된다 — personal.md 로 옮긴 뒤 재실행.
