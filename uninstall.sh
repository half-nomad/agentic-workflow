#!/bin/bash
# agentic-workflow 제거 스크립트 (WSL/Linux/macOS Bash)
...
if [ ! -f "$SOURCE_FILE" ]; then
    warn "agentic-workflow가 설치되지 않았습니다."
    exit 0
fi

SOURCE_PATH=$(cat "$SOURCE_FILE" | tr -d '\r\n')
info "설치된 소스 경로: $SOURCE_PATH"

if [ "$FORCE" = false ]; then
    read -p "agentic-workflow를 제거하시겠습니까? (y/N) " confirm
    [[ "$confirm" != "y" && "$confirm" != "Y" ]] && { info "제거가 취소되었습니다."; exit 0; }
fi
...
remove_installed_files() {
    ...
    [ "$removed_count" -gt 0 ] && success "$folder_name 에서 $removed_count 개 파일 제거됨"
}

info "설치된 파일들을 제거하는 중..."
...
if [ -n "$BACKUP" ]; then
    ...
    success "CLAUDE.md가 백업에서 복원되었습니다."
elif [ -f "$CLAUDE_MD" ]; then
    rm -f "$CLAUDE_MD"
    success "CLAUDE.md가 제거되었습니다."
fi

[ -f "$SOURCE_FILE" ] && rm -f "$SOURCE_FILE" && success ".agentic-workflow-source 파일이 제거되었습니다."
