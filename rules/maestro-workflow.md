...
정본은 **`skills/maestro/WORKFLOW.md` §사용자 보고** 다 — 용어를 푸는 방식, 결과 보고 4항, 용어 사전, 예시가 거기 있고 `/maestro` 진입 시 함께 로드된다.

여기 요약을 두지 않는 이유: 상주 파일에 같은 규약을 한 벌 더 두면 **한쪽만 고쳐질 때 옛 규약이 계속 주입된다.** 이 파일 머리말이 말하는 "상주는 활성화 조건과 절대 규칙만" 에도 보고 규약은 해당하지 않는다.
